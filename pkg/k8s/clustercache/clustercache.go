...
const (
	flagConfigmapNamespace    = "configmap-namespace"
	defaultConfigmapNamespace = "te-system"
)
...
type KubernetesClusterCache struct {
	...
	configMapWatch             WatchController
...
func NewKubernetesClusterCache(client kubernetes.Interface) ClusterCache {
	...
	configmapNamespace := strings.ToLower(viper.GetString(flagConfigmapNamespace))
	...
	kcc := &KubernetesClusterCache{
		...
		configMapWatch:             NewCachingWatcher(coreRestClient, "configmaps", &v1.ConfigMap{}, configmapNamespace, fields.Everything()),
	...
	wg.Add(15)
	go initializeCache(kcc.configMapWatch, &wg, cancel)
...
func (kcc *KubernetesClusterCache) Run() {
	...
	go kcc.configMapWatch.Run(1, stopCh)
...
func (kcc *KubernetesClusterCache) SetConfigMapUpdateFunc(f func(interface{})) {
	kcc.configMapWatch.SetUpdateHandler(f)
}
